...
- 5カテゴリ: 日常 / 気持ち / 都会 / 二人限定 / 沖縄（他に季節・平日/週末・ラッキー）
- **お題プール**: 現在 約2500個〜（拡張中、目標 10000+）
- **週のビンゴの対象**（`makeDailyItems`）: 日常 + 気持ち + 現在の季節（春/夏/秋/冬） + 平日 + 週末 + ラッキー1マス確定挿入
  - 季節は現在の月から自動判定（3-5月:春 / 6-8月:夏 / 9-11月:秋 / 12-2月:冬）
- グリッドサイズ 3×3 / 4×4 / 5×5
...
| **LINE個人** | 相手のLINE個人トーク | 帰宅系イベント（帰宅設定・会社出た・帰宅） |
| **LINEグループ** | 共有グループ | ゲージ同時MAX時のみ |

**方針**: 基本はPush通知。LINE通知は「帰宅系（相手に確実に届けたい）」と「ゲージMAX（グループで共有したいイベント）」のみ。
...
#### LINEグループ（target: 'group'）

| 発火元 | 条件 |
|--------|------|
| `closer.html` | ふたり同時MAXのとき |
