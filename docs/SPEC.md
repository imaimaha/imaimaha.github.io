...
- **カテゴリ別は前回の続きから** (2026-07-29〜): カテゴリを選ぶと、そのカテゴリの前回のカード（チェック状態つき）が Supabase から返る。**サイズ違いの扱い**: ユーザーがサイズボタンを押していないセッションでは既存カードのサイズに自動追従する（`state.sizeExplicit`。以前は 3×3 で作ったカードが再訪時の既定5×5と食い違い、進捗ごと捨てて新規生成されていた）
