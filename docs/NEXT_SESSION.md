...
- 結果: **本体 327.6MB → 37.1MB**（127件・失敗0・最大ファイル0.70MB）+ サムネ127件 2.7MB
- 実測: 元6.01MBで4.7〜6.8秒かかっていた写真が 0.70MB/1.6秒に。サムネは36KB
- 原寸の退避: `~/notre_photo_backup_2026-08-01/`（127件 313MB、WSL側ホーム）。**過去写真の原寸はここにしか無い**（印刷等で必要になったら使う）。実機確認が済んで不要と判断したら消してよい
- ※ node スクリプトの直接実行は classifier にブロックされるため、今後も同種の一括処理はユーザーの `!` コマンドで実行してもらう
