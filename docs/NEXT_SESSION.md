...
**残る露出（低リスク・要検討）**:
- ~~`location.html` の `BASES` に自宅の町名がベタ書き~~ → **同日対応済み**: `bases` テーブル（migration `20260912010000`、シードは直接SQL投入で町名をリポジトリに残さない）に移動し、location.html は起動時に DB から読む。テストも町名ベタ書きを排除。町名は履歴からも除去済み（履歴書き換え第2弾）
...
- **原因**: 位置取得 → Nominatim 逆引き → INSERT → 初訪問pt → 通知 をページ内 JS が順にやっていて、途中でページ遷移すると処理ごと消えていた
- **修正**: Edge Function **`checkin`** を新設（座標を受けて逆引き・登録・pt・通知をサーバー側で完結）。クライアントは `util.js: submitCheckin()` で **keepalive fetch** を1発投げるだけ（keepalive なので送信後にページを閉じても完了する）。index.html / location.html 両方を置換
- 位置取得も `enableHighAccuracy:false / maximumAge:60s` に変更（町名レベルには十分。取得が速いほど離脱の隙間が狭い）
- 残る隙間: **位置情報の取得中**（通常は一瞬）に離脱した場合だけは送信前なので登録されない
- 検証: claude アカウントで実座標を投げて `{ok, place_name:内藤町, first_visit:true}` を確認 → テスト行は削除済み。claude は profiles 行が無いので通知は飛ばない（実ユーザーにテスト通知が行かない、今後も使える手）
